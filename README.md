...
`netlify.toml` already configures everything (base `frontend`, build `rm -rf node_modules
package-lock.json && npm install && VITE_STATIC_API=1 npm run build`, publish `dist`, SPA fallback).
The lockfile **and** cached `node_modules` are wiped on CI because vite@8 (rolldown) needs a
platform-correct native binding that the macOS lockfile prunes (npm/cli#4828). To go live:
