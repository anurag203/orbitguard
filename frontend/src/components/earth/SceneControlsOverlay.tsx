...
export function SceneControlsOverlay({ controlsRef, invalidateRef, framing }: SceneControlsOverlayProps) {
  ...
  return (
    <div
      ...
      className="pointer-events-auto absolute bottom-4 right-4 z-[2] flex items-center gap-3 rounded-lg border border-hairline bg-surface/70 px-3 py-2 backdrop-blur-md"
    >
      ...
      <div className="flex items-center gap-2">
        <button type="button" className={BUTTON_CLASS} aria-label="zoom in" title="zoom in" onClick={run((c) => dolly(c, 0.85))}>
          <ZoomIn size={16} aria-hidden="true" />
        </button>
        <button type="button" className={BUTTON_CLASS} aria-label="zoom out" title="zoom out" onClick={run((c) => dolly(c, 1.18))}>
